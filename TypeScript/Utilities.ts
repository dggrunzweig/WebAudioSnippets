...
export function createWaveShaper(
    ctx: AudioContext, table: Float32Array, oversample = <OverSampleType>'4x'):
    WaveShaperNode {
    ...
    Promise<void> {
      return new Promise((resolve, reject) => {
        ...
        audio_ctx.audioWorklet.addModule(moduleURL)
            ...
            .catch((error) => {
              console.error(
                  `Error loading Audio Worklet module from ${moduleURL}`,
                  error);
              reject(error);
            });
      });
...
export function LoadMultipleWorkletModules(
    audio_ctx: AudioContext, urls: string[]): Promise<void> {
